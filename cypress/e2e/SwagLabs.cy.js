describe('SwagLabs', function () {
  ...
  xit('Add to Cart and cart icon functionality', function () {
      ...
      cy.wrap($el).within(() => {
        cy.get('.inventory_item_label').then(($label) => {
          ...
          if (productsToHandle.some(product => labelText.includes(product))) {
            cy.get('.pricebar').should('contain', '$')
            cy.get('button.btn_primary.btn_inventory').contains('ADD TO CART').click().wait(3000)
          }
        })
      })
    })
    ...
    cy.get('.cart_item').each(($el) => {
      cy.wrap($el).within(() => {
        cy.get('.cart_item_label').then(($label) => {
          ...
          if (productstoRemove.some(product => labelText.includes(product))) {
            cy.get('.item_pricebar').find('button').contains('REMOVE').click().wait(3000)
     ...
     cy.get('.cart_footer').find('a').contains('CHECKOUT').click().wait(3000)
     cy.get('.subheader').should('have.text','Checkout: Your Information')
     cy.get('input[type="submit"]').click().wait(3000)
     cy.get('h3[data-test="error"]').should('have.text','Error: First Name is required').wait(3000)
     cy.get('#first-name').type('Test User 23').should('have.value','Test User 23')
     cy.get('input[type="submit"]').click().wait(3000)
     cy.get('h3[data-test="error"]').should('have.text','Error: Last Name is required')
     cy.get('#last-name').type('Cypress').should('have.value','Cypress').wait(3000)
     cy.get('input[type="submit"]').click().wait(3000)
     cy.get('h3[data-test="error"]').should('have.text','Error: Postal Code is required')
     cy.get('#postal-code').type('548411').should('have.value','548411').wait(3000)
     cy.get('input[type="submit"]').click()
     cy.get('.summary_info_label').contains('Payment Information:').should('have.text','Payment Information:')
     cy.get('.summary_value_label').should('have.text','SauceCard #')
   })
})
